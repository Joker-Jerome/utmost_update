...
bim <- na.omit(bim)
gene_names <- list.files(paste0("/gpfs/loomis/scratch60/zhao/zy92/GTEX/expr_gtex_lnc_updated/chr",chr,"/"))
remain <- c()
...
#  gene_in_gtex <- genes_gtex$V1[grep(tmp[1,1],genes_gtex$V1)] 
  ...
  dir.create(paste0("/gpfs/loomis/project/zhao/zy92/GTEX/genotype/cis_loc_lnc/chr",chr,"/", gene_name_gtex,"/"), recursive = T, showWarnings = F)
  tmp_file <- paste0("/gpfs/loomis/project/zhao/zy92/GTEX/genotype/cis_loc_lnc/chr",chr,"/", gene_name_gtex,"/", gene_name_gtex, ".txt")
  write.table(tmp_df,file=tmp_file,col.names=F,row.names=F,sep="\t",quote=F)
 
}

write.table(remain,file=paste0("/gpfs/loomis/project/zhao/zy92/GTEX/genotype/cis_loc_lnc/chr",chr,"_remainGenes.txt"),col.names=F,row.names=F,sep="\t",quote=F)
print("INFO: complete!")
