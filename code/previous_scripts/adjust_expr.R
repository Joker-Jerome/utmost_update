...
for(chr in 9:22){
	...
	for(k in 1:length(glist)){
		...
		#k = 1; 
			...
			Yt = dir(paste0("/ysm-gpfs/pi/zhao/from_louise/yh367/GTEX/expr_gtex_portal/chr", chr, "/", g, "/"))
			
			dir.create(paste0("/ysm-gpfs/pi/zhao/from_louise/yh367/GTEX/adjusted_expr/chr", chr), showWarnings = FALSE)
			dir.create(paste0("/ysm-gpfs/pi/zhao/from_louise/yh367/GTEX/adjusted_expr/chr", chr, "/", g), showWarnings = FALSE)
			setwd(paste0("/ysm-gpfs/pi/zhao/from_louise/yh367/GTEX/adjusted_expr/chr", chr, "/", g))
			## expr files ##
			Y = list()
			for(t in 1:length(Yt)){
				Y[[t]] = read.table(paste0("/ysm-gpfs/pi/zhao/from_louise/yh367/GTEX/expr_gtex_portal/chr", chr, "/", g, "/", Yt[t]), header=F)
			}
